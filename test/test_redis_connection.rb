...
describe Sidekiq::RedisConnection do
    ...
    describe 'logging redis options' do
      it 'redacts credentials' do
        options = {
          ...
          sentinels: [
            { host: 'host1', port: 26_379, password: 'secret' },
            { host: 'host2', port: 26_379, password: 'secret' },
            { host: 'host3', port: 26_379, password: 'secret' }
          ],
          password: 'secret'
        }
  ...
  describe '.determine_redis_provider' do
    ...
    def with_env_var(var, uri, skip_provider = false)
      vars = %w[REDISTOGO_URL REDIS_PROVIDER REDIS_URL] - [var]
      vars.each do |v|
